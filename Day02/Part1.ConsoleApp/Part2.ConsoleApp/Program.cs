...
namespace Part2.ConsoleApp
{
    class Program
    {
        ...
        {
            int total = 0;
            EvenlyDividesFinder d = new EvenlyDividesFinder();

            foreach (var row in numberList)
            {
                var rowAsInt = row.Split(" ", StringSplitOptions.RemoveEmptyEntries)
                    .Select(x => int.Parse(x));
                total += d.Get(rowAsInt);
            }
